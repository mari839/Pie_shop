...
namespace BethanyPieShop.Models
{
    public class ShoppingCart : IShoppingCart
    {
        ...
        public static ShoppingCart GetCart(IServiceProvider services) //?
        {
            ...
            string cartId = session?.GetString("CartId") ?? Guid.NewGuid().ToString(); //check if there is already a value called CartId for the incoming user, if not we create new GUID, if there is we take CartId value from session

            session?.SetString("CartId", cartId); //we set value of the cartid to the session

            return new ShoppingCart(context) { ShoppingCartId = cartId }; //? what is CartId in session? why are we creating new class
        }
        ...
        public void AddToCart(Pie pie)
        {
            var shoppingCartItem =
                    _bethanysPieShopDbContext.ShoppingCartItems.SingleOrDefault(
                        s => s.Pie.PieId == pie.PieId && s.ShoppingCartId == ShoppingCartId); //we check if in shopping cart we have the same id as in Pie
        ...
        public int RemoveFromCart(Pie pie)
        {
            ...
            if (shoppingCartItem != null)
            {
                ...
                {
                    shoppingCartItem.Amount--;
                    localAmount = shoppingCartItem.Amount; //rat gvinda localAmount
                }
                else
                {
